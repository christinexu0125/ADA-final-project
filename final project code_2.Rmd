...
```{r}
# Logistic regression for the association between depression and smoking without smoking*year interaction
deplogit01 <- glm(ymdeyr ~ cigyr+year,family="binomial",data=nsduh)
summary(deplogit01)


# Logistic regression for the association between depression and smoking with smoking*year interaction
deplogit02 <- glm(ymdeyr ~ cigyr+year+cigyr*year,family="binomial",data=nsduh)
summary(deplogit02)

#calculate and print ORs and 95% CIs  
  depOR<-exp(cbind(OR = coef(deplogit02), confint(deplogit02))) #calculate ORs and 95% CIs
...
#compare models without smoking*year interaction to that with smoking*year interaction using LR test
install.packages("lmtest")
library(lmtest)

lrtest(deplogit01, deplogit02)
```
```
